# AGENTS.md - AI Agent Development Guide for target-jsonl
...
This target follows the Singer specification and uses the Meltano Singer SDK to load data into JSONL.

### Key Components

1. **Target Class** (`target_jsonl/target.py`): Main entry point, defines configuration and sinks
1. **Sink Class** (`target_jsonl/sinks.py`): Handles data loading and processing
...
1. Override sink methods in `target_jsonl/sinks.py`
...
cat sample_data.singer | target-jsonl --config config.json

# Test with a tap
tap-something --config tap_config.json | target-jsonl --config config.json
```
...
1. Update `config_jsonschema` in `target_jsonl/target.py`
...
```bash
# .env.example
TARGET_JSONL_API_URL=https://api.example.com
TARGET_JSONL_API_KEY=your_api_key_here
TARGET_JSONL_BATCH_SIZE=1000  # New setting
```
