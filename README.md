...
```bash
uv tool install target-jsonl
```

Install from GitHub:

```bash
uv tool install git+https://github.com/ORG_NAME/target-jsonl.git@main
```
...
tap-smoke-test | target-jsonl --config /path/to/target-jsonl-config.json
```
...
You can also test the `target-jsonl` CLI interface directly using `uv run`:

```bash
uv run target-jsonl --help
```
